...
If \verb|n| is an integer, it's also possible to represent \verb|Nu n| just by \verb|n|, so:
...
\begin{sgcode}
\bash{let g = Op [Nu 1, LE 2 [Nu 3]] [Nu 4]}
\bash{g}
\out{<1,<^2|3>|4>}\\
\bash{leftOp g}
\out{[1,<^2|3>]} \\
...
\bash{goto [1] g}
\out{<^0|<-1|-2>>} \hsklcmt{same as lop 1 g} \\
\bash{goto [1,-1] g}
\out{<-1|-2>} \hsklcmt{same as rop 1 . lop 1 \$ g} \\
\bash{goto [1,-1,1] g}
\out{-1} \hsklcmt{same as lop 1 . rop 1 . lop 1 \$ g}
\end{sgcode}
...
Function \verb|guaranteed| checks if a given game $G$ description is guaranteed, ie $G \in \SG$:
...
\begin{itemize} \setlength\itemsep{0.1em}
  ...
  \item \verb|ls_ game| left stop pass-allowed of game
  \item \verb|rs_ game| right stop pass-allowed of game
\end{itemize}

\begin{sgcode}
\bash{let g = LE 5 [Op [Op [Nu 10] [Nu 1]] [Nu (-3)]]}
\bash{rs g}
\out{1.0} \\
\bash{ls g}
\out{5.0}
...
(this includes the possibility that the two games are not comparable).
...
\out{True} \\
\bash{Nu 5 >== LE (-4) [Nu 6]}
\out{False} \hsklcmt{only checks if >==, not if <==}
\end{sgcode}

The third set compares two games, as before, but checks if they are comparable. If they
are comparable it returns \verb|Just <result>|, if not it returns \verb|Nothing|.
...
At this moment, the functions include comparing games with the same game tree, using the monotone principle and
comparing with atomic substitution. 
...
\out{<<2|2,<2|2>>,<1|1>|<<<^4|4>|<4|^4>>|-5>>} \\
\bash{canonize g}
\out{<^2|^4>} \hsklcmt{a quick alternative is to evaluate g+0}
\end{sgcode}
